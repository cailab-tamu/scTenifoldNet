...
#' @param td_nDim An integer value. Defines the number of rank-one tensors used to approximate the data using CANDECOMP/PARAFAC (CP) Tensor Decomposition. 
#' @param td_maxIter An integer value. Defines the maximum number of iterations if error stay above td_Tol.
#' @param td_Tol A decimal value between 0 and 1. Defines the relative Frobenius norm error tolerance.
...
scTenifoldNet <- function(X, Y, qc_minLibSize = 1000, qc_removeOutlierCells = TRUE,
                          ...
                          nc_q = 0.05, td_nDim = 3, td_maxIter = 1e6, td_Tol = 1e-5, ma_nDim = 30){
  ...
  # CANDECOMP/PARAFRAC Tensor Decomposition
      # for(M in c('I','3d','4d')){
  set.seed(1)
  tensorOut <- tensorDecomposition(xList, yList, d = td_nDim)
