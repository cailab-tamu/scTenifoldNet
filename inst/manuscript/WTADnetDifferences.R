...
library(Seurat)
dC <- read.csv('results/sym10X500WTAD_Itensor_Dalignment.csv', row.names = 1)
...
Y <- WTAD$tensorNetworks$X
O <- WTAD$tensorNetworks$Y

Y <- as.matrix(Y)
O <- as.matrix(O)

Y[abs(Y) < quantile(abs(Y),1-(1e-5))] <- 0
O[abs(O) < quantile(abs(O),1-(1e-5))] <- 0
...
mY <- read.csv('datasets/WTAD/Xw2.txt', header = FALSE)
rownames(mY) <- readLines('datasets/WTAD/genelist.txt')

mO <- read.csv('datasets/WTAD/Xa2.txt', header = FALSE)
rownames(mO) <- readLines('datasets/WTAD/genelist.txt')
...
png('figures/wtadDiffNetworks.png', width = 4000, height = 2000, res = 300, pointsize = 20, bg = NA)
